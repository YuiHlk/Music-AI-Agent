...
2026-07-17 模型接入已从DeepSeek专用配置重构为通用`llm` Profile。服务端通过`LLM_PROVIDER`、`LLM_BASE_URL`、`LLM_MODEL`和`LLM_API_KEY`选择OpenAI兼容模型；应用层仍只依赖LangChain4j `ChatModel`。MCP新增`generate_guitar_riff_from_constraints`，允许外部模型使用自己的Key完成意图解析并直接提交结构化约束，后端不得在该路径再次调用服务器模型。网页端多用户BYOK需要先建设用户身份、项目所有权和加密凭据存储，当前不得保存用户明文Key。

内部`MusicCreatorAgent`与`MusicProjectService`位于同一进程，必须通过`MusicCreationTools`直接调用应用用例，不得通过MCP进行HTTP自调用。内部Agent生成工具接收模型已推断的结构化字段，避免同一请求再次进入模型驱动`RequirementParser`；异步生成与重写返回任务后，应先查询任务达到`COMPLETED`再校验或查询导出物。
